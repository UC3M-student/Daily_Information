...
def scrape_okdiario_headlines(limit=5):
    ...
    headlines = [h.get_text(strip=True) for h in soup.find_all(["h1", "h2"])][:limit]
    return headlines
...
def scrape_energy_prices():
    ...
    if table:
        ...
        print("⚠️ Warning: Energy prices table not found.")
...
def get_madrid_hourly_forecast(hours_to_show=12):
    ...
    data = []
    if not table:
        print("⚠️ Warning: Weather table not found.")
        return pd.DataFrame(data)
    ...
    for row in rows:
        ...
        if "°C" not in temp: temp = temp.replace(" ", "") + "°C"
        if "°C" not in feels: feels = feels.replace(" ", "") + "°C"
...
def color_percentages(html):
    html = re.sub(r'>(-\d+\.?\d*%)<',
                  r'><span style="color:#d62728;font-weight:600;">\1</span><', html)
    html = re.sub(r'>(\+?\d+\.?\d*%)<',
                  r'><span style="color:#2ca02c;font-weight:600;">\1</span><', html)
    return html
...
def generate_html_report(headlines, energy_df, market_df, stocks_df, forecast_df):
    ...
    <head>
        ...
        <style>
            ...
            header {{ background: linear-gradient(90deg,#1f4e79,#163a5f); color: white; padding: 30px; text-align: center; }}
            .container {{ padding: 40px; }}
            .card {{ background: white; padding: 25px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); margin-bottom: 40px; }}
            table.styled {{ border-collapse: collapse; width: 100%; font-size: 14px; }}
            table.styled th {{ background: #163a5f; color: white; padding: 12px; text-align: left; }}
            table.styled td {{ padding: 10px; border-bottom: 1px solid #eee; }}
            table.styled tr:nth-child(even) {{ background-color: #f8fbff; }}
        </style>
    </head>
    <body>
    <header>
    ...
    <div class="container">
        <div class="card"><h2>📰 Headlines</h2><ul>{''.join(f"<li>{h}</li>" for h in headlines)}</ul></div>
        <div class="card"><h2>⚡ Energy Prices</h2>{energy_html}</div>
        <div class="card"><h2>🏢 EU Market Cap</h2>{market_html}</div>
        <div class="card"><h2>📈 Global Markets</h2>{stocks_html}</div>
        <div class="card"><h2>🌤 Madrid Forecast</h2>{forecast_html}</div>
    </div>
    </body>
    </html>
    """
